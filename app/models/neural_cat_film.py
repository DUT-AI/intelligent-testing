...
from app.models.building_blocks import (
    ...
    NeuralCATDecoder,
    NeuralCATPredictor,
    QuestionFeatureFiLMFusion,
)
from app.domain.entities.cat_entities import CATModelOutput


class NeuralCATEngineFiLM(BaseCATEngine):
    """
    FiLM-modulated Neural CAT Engine Model.
    Fuses question text embeddings and tabular features using Feature-wise Linear Modulation (FiLM).
    Provides higher capacity for tabular feature interaction.
    """
    ...
    def __init__(
        ...
        # 1. Feature Fusion Module using FiLM
        self.d_x = d_embedding  # Keep output of fusion at d_embedding size (e.g. 1024)
        self.fusion = QuestionFeatureFiLMFusion(
            d_embedding=d_embedding, d_features=d_features, d_out=self.d_x
        )
    ...
    def forward(
        ...
        """
        # 1. Fuse embeddings and tabular features using FiLM
