...
def main():
    ...
    # 3. Giả lập 1 học sinh Trung bình (True Theta = 0.2)
    theta_true = 0.2
    num_steps = 30
    max_seq_len = 50
    max_c = 23
    ...
    print("\nSimulating student response trajectory...")
    theta_t = 0.0
    ...
    # 4. Xác định các concept mục tiêu từ lịch sử thi thực tế
    active_correct_c = None
    active_incorrect_c = None
    ...
    # Chọn concept active_correct và active_incorrect
    for c, occs in concept_occurrences.items():
        has_correct = any(r == 1.0 for _, r in occs)
        has_incorrect = any(r == 0.0 for _, r in occs)
        if has_correct and active_correct_c is None:
            active_correct_c = c
        if has_incorrect and active_incorrect_c is None:
            active_incorrect_c = c
            
    if active_correct_c is None:
        active_correct_c = list(all_tested_concepts)[0] if all_tested_concepts else 1
    if active_incorrect_c is None:
        active_incorrect_c = list(all_tested_concepts)[1] if len(all_tested_concepts) > 1 else 2
    ...
    print(f"\nTarget concepts for tracking:")
    print(f"  - Concept {active_correct_c} (Active, Correct): Occurrences = {concept_occurrences.get(active_correct_c, [])}")
    print(f"  - Concept {active_incorrect_c} (Active, Incorrect): Occurrences = {concept_occurrences.get(active_incorrect_c, [])}")
    print(f"  - Concept {control_c} (Control, Not Tested): Never appeared.")

    # 5. Theo dõi điểm số của 3 concept này qua từng bước t từ 1 đến 30
    # Lần này ta sẽ trích xuất trực tiếp giá trị tích lũy trong theta_prev qua các bước
    trajectory_data = {
        active_correct_c: [],
        active_incorrect_c: [],
        control_c: []
    ...
    # Chạy lại từng step để trích xuất theta_prev tích lũy tại cuối bước đó
    for step in range(num_steps):
        x_emb_seq = torch.zeros(1, max_seq_len, 1024, device=device)
        x_feat_seq = torch.zeros(1, max_seq_len, 22, device=device)
        r_seq = torch.zeros(1, max_seq_len, device=device)
        ...
        with torch.no_grad():
            ...
            # Khởi tạo lại vòng lặp cập nhật của Decoder để trích xuất ma trận theta_prev tích lũy tại bước 'step'
            h_projected = model.model.decoder.proj_h(h)
            alpha = model.model.decoder._get_damping_schedule(max_seq_len, device)
            
            # Bắt đầu với theta_0
            theta_prev = model.model.decoder.theta_0.unsqueeze(0)  # (1, K+1, d_h)
            
            # Chạy qua các bước từ 0 đến step để cập nhật ma trận theta_prev
            for t in range(step + 1):
            ...
            # Trích xuất mastery trực tiếp từ theta_prev tích lũy
            for c in [active_correct_c, active_incorrect_c, control_c]:
                theta_c = theta_prev[0, c, :]  # shape (d_h,)
    ...
    # 6.1. Vẽ đường của Concept Active - Correct
    plt.plot(steps, trajectory_data[active_correct_c], label=f"Concept {active_correct_c} (Làm đúng)", color="green", linewidth=2.5)
    correct_steps = [occ[0] + 1 for occ in concept_occurrences.get(active_correct_c, []) if occ[1] == 1.0]
    correct_scores = [trajectory_data[active_correct_c][s - 1] for s in correct_steps]
    plt.scatter(correct_steps, correct_scores, color="green", marker="o", s=120, edgecolors="black", zorder=5, label="Làm ĐÚNG câu chứa concept")
    
    # 6.2. Vẽ đường của Concept Active - Incorrect
    plt.plot(steps, trajectory_data[active_incorrect_c], label=f"Concept {active_incorrect_c} (Làm sai)", color="red", linewidth=2.5)
    incorrect_steps = [occ[0] + 1 for occ in concept_occurrences.get(active_incorrect_c, []) if occ[1] == 0.0]
    incorrect_scores = [trajectory_data[active_incorrect_c][s - 1] for s in incorrect_steps]
    plt.scatter(incorrect_steps, incorrect_scores, color="red", marker="X", s=120, edgecolors="black", zorder=5, label="Làm SAI câu chứa concept")
    
    # 6.3. Vẽ đường của Concept Control (không được thi)
    plt.plot(steps, trajectory_data[control_c], label=f"Concept {control_c} (Đối chứng - Không được thi)", color="gray", linestyle="--", linewidth=2)
    
    plt.title("Lịch Sử Năng Lực Tích Lũy Thực Tế Của Từng Concept (Step-by-Step Concept Trajectory)", fontsize=14, fontweight="bold")
    plt.xlabel("Số câu hỏi đã trả lời (Step t)", fontsize=12)
    plt.ylabel("Điểm số Concept (Thang điểm 10)", fontsize=12)
    plt.ylim(0.0, 10.5)
    plt.xlim(0.5, 30.5)
    ...
    textstr = '\n'.join((
        r'$\bf{Quan\ sat\ quy\ luat\ moi:}$',
        r'- Concept $\bf{khong\ thi}$ (duong dut net): Dung im tuyet doi, khong dao dong.',
        r'- Concept $\bf{DUNG}$ (marker O): Chi tang khi gap marker, cac buoc khac di ngang.',
        r'- Concept $\bf{SAI}$ (marker X): Chi giam khi gap marker, cac buoc khac di ngang.',
        r'--> Mo hinh cap nhat cuc ky chinh xac va doc lap giua cac concept!'
    ))
    props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
    plt.gca().text(0.02, 0.05, textstr, transform=plt.gca().transAxes, fontsize=9.5,
            verticalalignment='bottom', bbox=props)

    plt.tight_layout()
    plot_path = os.path.join(artifacts_dir, "concept_trajectory.png")
    plt.savefig(plot_path, dpi=150)
    plt.close()
    
    print(f"\nSaved concept trajectory plot to {plot_path}")
