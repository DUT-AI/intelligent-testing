...
class StudentSequenceDataset(Dataset):
    def __init__(self, sequences, question_embeddings, question_concepts, question_features=None, max_seq_len=200, K=1175):
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train LitNeuralCAT on student sequence data")
    parser.add_argument("--model_type", type=str, default="base", choices=["base", "optimized"], help="Which model version to train")
    parser.add_argument("--epochs", type=int, default=10, help="Number of training epochs")
    parser.add_argument("--batch_size", type=int, default=64, help="Batch size for training")
    parser.add_argument("--lr", type=float, default=1e-3, help="Learning rate")
    parser.add_argument("--lambda_reg", type=float, default=0.1, help="Guess/slip regularization strength")
    parser.add_argument("--max_seq_len", type=int, default=200, help="Maximum sequence length")
    parser.add_argument("--limit_train_batches", type=float, default=1.0, help="Fraction/count of training batches to use")
    parser.add_argument("--limit_val_batches", type=float, default=1.0, help="Fraction/count of validation batches to use")
...
def main():
    ...
    try:
        ...
        train_sequences = db_session.query(StudentSequence).filter(
            StudentSequence.dataset_type == "train_valid"
        ).all()
        val_sequences = db_session.query(StudentSequence).filter(
            StudentSequence.dataset_type == "test"
        ).all()
        print(f"Loaded {len(train_sequences)} training sequences and {len(val_sequences)} validation sequences.")
    ...
    if checkpoint_callback.best_model_path:
        ...
        torch.save(model.state_dict(), f"checkpoints/neural_cat_{args.model_type}_last.ckpt")
        print(f"Saved last model weights to checkpoints/neural_cat_{args.model_type}_last.ckpt")
