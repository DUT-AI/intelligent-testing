...
class LitNeuralCAT(L.LightningModule):
    ...
    def forward(self, x: torch.Tensor, r: torch.Tensor, T_time: torch.Tensor, Q: torch.Tensor, padding_mask: torch.Tensor = None):
        return self.model(x, r, T_time, Q, padding_mask)
    ...
    def training_step(self, batch, batch_idx):
        ...
        (x, r, T_time, Q, padding_mask)
        """
        x, r, T_time, Q, *rest = batch
        # Extract padding mask if provided, else default to None
        padding_mask = rest[0] if len(rest) > 0 else None
        
        # Forward pass
        P, g, s = self(x, r, T_time, Q, padding_mask)
